...
def get_envelope(slce, window=10):
    padded = np.concatenate(
        ([slce[0]] * window, slce, [slce[-1]] * window)
    )
...
def lbp_entropy(patch, P, R):
    ...
    hist, _ = np.histogram(
        lbp.ravel(), bins=P + 2, range=(0, P + 1), density=True
    )
...
class Detection:
    ...
    def process_dem(self, path, sid, folder_path):
        ...
        for i, res in enumerate(stream, start=1):
            ...
            self.progress[sid] = min(
                50 + math.floor(i * 49 / total_patches), 99
            )
        ...
        for cx, cy in pts:
            ...
            ci = int(round(cx))
            patch = arr[
                ri - half : ri + half, ci - half : ci + half
            ]
            if patch.shape != (
                Config.PATCH_SIZE,
                Config.PATCH_SIZE,
        ...
        for pt in pts:
            if not any(
                math.hypot(pt[0] - u[0], pt[1] - u[1]) < thr
                for u in unique_pts
            ):
        ...
        # 7. TPI
        tpi_map = compute_tpi(
            arr, Config.PATCH_SIZE * Config.TPI_WINDOW_MULTIPLIER
        )
        tpis = [
            tpi_map[int(round(cy)), int(round(cx))]
            for cx, cy in pts
        ]
        ...
        entropies = [
            lbp_entropy(
                arr[
                    int(round(cy)) - half : int(round(cy)) + half,
        ...
        ent_thr = np.percentile(
            entropies, Config.LBP_ENTROPY_PERCENTILE
        )
        final_pts = [
            pt for pt, ent in zip(pts, entropies) if ent <= ent_thr
        ]
        ...
        with open(
            ...
            for idx, (cx, cy) in enumerate(final_pts, start=1):
                ...
                gy = top - cy * cellsize
                writer.writerow(
                    [f"Candidate_{idx}", f"{gx:.3f}", f"{gy:.3f}"]
                )

        gif = f"animation_{sid}.gif"
        with imageio.get_writer(
            os.path.join(folder_path, gif), mode="I", fps=1
        ) as w:
            for fr in frames:
                w.append_data(fr)
    ...
    def _extract_patches(self, img, out_dir, sid):
        ...
        for i, (r, c) in enumerate(coords, start=1):
            ...
            fn = os.path.join(
                out_dir, f"patch_{r}_{c}_{Config.PATCH_SIZE}.png"
            )
            save_rgb(patch, fn)
