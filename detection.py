...
def get_slice(img, cell_size, max_slope=Config.MAX_SLOPE_PERCENT):
    ...
    return all(check_trend(slc) for slc in dirs.values())

def get_envelope(slce, window=10):
    padded = np.concatenate(([slce[0]]*window, slce, [slce[-1]]*window))
    env = np.convolve(padded, np.ones(window), 'same')/window
    return env[window:-window]

def get_slice_array(img, direction):
    if direction=='h':    return img[img.shape[0]//2, :]
    if direction=='v':    return img[:, img.shape[1]//2]
    if direction=='dlr':  return img.diagonal()
...
def remove_trend(slce, cell_size):
    ...
    trend=(x-p1[0])*(p2[1]-p1[1])/(p2[0]-p1[0])+p1[1]
    return slce-trend

def filter_by_skew(img, cell_size,
...
def slope_variance(img, cell_size, thresh=Config.VARIANCE_THRESH):
    vars_list=[]
    for d in ['h','v','dlr','drl']:
        slc = get_slice_array(img, d)
        detr = remove_trend(slc, cell_size)
...
def filter_by_circularity(patch,
                          level=Config.CIRCULARITY_LEVEL,
                          circ_thresh=Config.CIRCULARITY_THRESH):
    p=(patch-patch.min())/(patch.max()-patch.min()+1e-6)
    _, bw = cv2.threshold((p*255).astype(np.uint8),
                          int(level*255), 255, cv2.THRESH_BINARY)
...
def compute_curvatures(patch, cell_size):
    Z   = patch
    Zxx = (Z[2:,1:-1]-2*Z[1:-1,1:-1]+Z[:-2,1:-1])/(cell_size**2)
    Zyy = (Z[1:-1,2:]-2*Z[1:-1,1:-1]+Z[1:-1,:-2])/(cell_size**2)
    return Zxx + Zyy

def filter_by_curvature(patch, cell_size,
                        H_thresh=Config.CURVATURE_THRESH):
    H  = compute_curvatures(patch, cell_size)
    cy, cx = H.shape[0]//2, H.shape[1]//2
    win = H[max(cy-11,0):cy+12, max(cx-11,0):cx+12]
    return win.mean() > H_thresh

def compute_tpi(dem, window):
    return dem - uniform_filter(dem, size=window, mode='nearest')

def lbp_entropy(patch, P, R):
    norm = (patch-patch.min())/(patch.max()-patch.min()+1e-6)
    img8 = (norm*255).astype(np.uint8)
    lbp  = local_binary_pattern(img8, P, R, method='uniform')
    hist,_ = np.histogram(lbp.ravel(),
                         bins=P+2, range=(0,P+1), density=True)
    return -np.sum(hist*np.log2(hist+1e-6))

# ----------------------------
#  Основной класс Detection
# ----------------------------
class Detection:
    def __init__(self):
    ...
    def process_dem(self, path, sid, folder_path):
        ...
        total_patches = self._extract_patches(stretched,
                                              patch_dir, sid)
        ...
        for i, res in enumerate(stream, start=1):
            ...
            _, r_str, c_str, _ = os.path.basename(res.path).split('_')
            r0, c0 = int(r_str), int(c_str)
            for x1,y1,x2,y2 in res.boxes.xyxy.cpu().numpy():
                cx = (x1+x2)/2 + c0
                cy = (y1+y2)/2 + r0
                detected.append((cx, cy))
            self.progress[sid] = min(50 + math.floor(i*49/total_patches), 99)

        # Квантильная фильтрация
        valid = arr[arr!=ndv] if ndv is not None else arr[~np.isnan(arr)]
        low_q, high_q = np.percentile(valid, [0, Config.QUANTILE_PCT])
        pts = [(cx, cy) for cx, cy in detected
               if arr[int(round(cy)), int(round(cx))] >= high_q]

        half = Config.PATCH_SIZE // 2
        filtered = []

        # 1. Slope + проверка размера
        for cx, cy in pts:
        ...
        pts, filtered = filtered, []

        # 2. Proximity
        thr = Config.PROXIMITY_M / cellsize
        unique_pts = []
        for pt in pts:
            if not any(math.hypot(pt[0]-u[0], pt[1]-u[1])<thr
                       for u in unique_pts):
        ...
        # 7. TPI
        tpi_map = compute_tpi(arr,
                              Config.PATCH_SIZE * Config.TPI_WINDOW_MULTIPLIER)
        tpis = [tpi_map[int(round(cy)), int(round(cx))] for cx, cy in pts]
        tpi_thr = np.percentile(tpis, Config.TPI_PERCENTILE)
        ...
        # 8. LBP
        entropies = [lbp_entropy(
            arr[int(round(cy))-half:int(round(cy))+half,
                int(round(cx))-half:int(round(cx))+half],
            Config.LBP_P, Config.LBP_R) for cx, cy in pts]
        ent_thr = np.percentile(entropies,
                                 Config.LBP_ENTROPY_PERCENTILE)
        final_pts = [pt for pt, ent in zip(pts, entropies)
                     if ent <= ent_thr]
        ...
        fig.savefig(os.path.join(folder_path, png), dpi=100)
        plt.close(fig)

        csv_name = f'coords_{sid}.csv'
        with open(os.path.join(folder_path, csv_name),
            ...
            for idx, (cx, cy) in enumerate(final_pts, start=1):
                gx = xll + cx*cellsize
                gy = top - cy*cellsize
                writer.writerow([f'Candidate_{idx}',
                                 f'{gx:.3f}', f'{gy:.3f}'])

        gif = f'animation_{sid}.gif'
        with imageio.get_writer(os.path.join(folder_path, gif),
                                mode='I', fps=1) as w:
            for fr in frames:
                w.append_data(fr)

        self.results[sid]    = {'png': png, 'csv': csv_name, 'anim': gif}
